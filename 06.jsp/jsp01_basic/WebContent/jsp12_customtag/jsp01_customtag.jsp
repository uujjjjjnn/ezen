...
<!-- 
	...
	프로그램을 개발하다 보면 JSP액션태그나 JSTL태그만으로 부족할 때가 있는데 이를 지원하기 위해 JSP는 새로운 Tag를 만들어서 사용할 수 있도록 하는 기능을 제공한다.
	사용자가 원하는 목적에 맞게 작성한 태그를 커스텀태그라고 한다.
	
	1. 커스텀태그의 장점
	
		a. 재사용 : 한 번 작성한 커스텀태그는 어떤 JSP컨테이너에서도 사용이 가능하다.
		b. 쉽고 간단한 JSP코드를 작성할 수 있다. 자바코드에 익숙하지 않은 개발자들도 커스텀태그를 사용하면 보다 쉽게 JSP페이지를 작성할 수 있다.
	...
	3. 태그디렉티브의 속성
		...
		b. body-content 				: body내용의 종류를 정의. empty, tagdependant, scriptless의 세가지 값을 사용할 수 있다. 기본값은 scriptless
			1) empty 		: body가 비어 있어야 한다
			2) tagdependant : tag자체의 구현에 종속되어 있다
			3) scriptless   : 스크립트요소를 정의할 수 없다
		
		c. dynamic-attribute : 동적으로 속성을 정의. 속성<key, value>를 저장하는 Map객체를 page범위의 속성에 저장할 때 사용할 이름을 정의
		...
		i. trimDirectiveWhiteSpace : page디렉티브의 속성과 동일. 공백문자를 제거여부
