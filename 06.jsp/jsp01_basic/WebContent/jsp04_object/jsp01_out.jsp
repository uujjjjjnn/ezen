...
<!-- 
	...
	6. application(javax.servlet.jsp.ServletContext) : 웹배플리케이션에 대한 정보를 저장
	7. page(java.lang.Ojbect) : jsp페이지를 구현한 자바클래스의 인스턴스(객체)
	8. config(javax.servlet.ServletConfig) : jsp페이지에 대한 설정정보를 저장
	9. exception(java.lang.Throwable) : Exception객체, 에러페이지라고 정의된 페이지에서만 사용 가능
 ...
 <!-- 
 	...
 	2) out객체의 buffer관련메서드
		...
		c. clear() 		   : 버퍼전체를 비움, 이미 flush가 되어있다면 익셉션이 발생
		d. clearBuffer()   : 버퍼전체를 비움, 이미 flush가 되어있다면 익셉션이 발생하지 않음
		e. flush() 		   : 버퍼를 flush처리
