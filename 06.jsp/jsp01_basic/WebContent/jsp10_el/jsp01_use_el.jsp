...
<!-- 
	...
	표현언어는 다른 형태의 스크립트언어로서 스크립트요소 중 하나이다. 표현언어는 표현식보다 간결하고 코딩하기가 편하기 때문에
	많이 사용한다
	
	EL은 JSTL(JSP Standard Tag Library)규약에 나오는 내용으로서 JSP2.0부터 EL이 JSTL에 포함되었고 EL은 값을 표현하는데 
	사용하는 언어로 JSP의 스크립트요소를 보완하는 역할을 한다.
	...
	A. EL의 구성
		...
		EL은 "$ {expression}"의 형태로 사용할 수 있다. EL은 JSP의 스크립트요소(스크립트릿, 표현식, 선언부)를 제외한 나머지 부분에서 사용할 수 있다.
		JSP2.1버전부터는 "$ {}"형식과 "# {}"형식의 구분을 지원하고 있는데 차이점은 EL의 값을 언제 생성하느냐에 차이가 있다.
		$ {}은 구문을 분석할 때 바로 값을 계산하지만 # {}은 실제로 값이 사용될 때 계산이 된다. 
	...
	C. EL의 기본객체
		...
		8. header 			: request.getHeader()와 동일기능
	...
	D. 객체의 사용
		...
		<표현>.<값> 또는 <표현>[표현] 형식으로 접근하는데 접근과정은 다음과 같은 순서로 변환된다.
		...
		4. <값1>이 Map, List, Array일 경우
			...
			b. List, Array : <값2>가 정수값인지 여부를 검사(정수가 아니면 에러발생) 후 <값1>.<값2> 또는 Arrays.get(<값1>, <값2>)의 결과를 리턴
