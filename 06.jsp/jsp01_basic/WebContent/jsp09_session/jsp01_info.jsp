...
<!-- 
	...
	즉, cookie가 클라이언트 측의 데이터 저장소라면 session은 서버 측의 저장소이다.
	...
	A. session 기본객체
		...
		session객체는 request객체와 마찬가지로 속성을 제공하고 setAttribute, getAttribute메서드를 사용하여 session의 속성값을 저장하거나
		읽을 수가 있다. 다만, session객체는 session만의 고유정보를 제어할 수 있는 메서드가 추가로 제공된다.
	...
	B. session 객체의 속성
		...
		한 번 생성된 session은 지정한 유효시간동안 유지가 되기 때문에 웹애플리케이션을 실행하는 동안 지속적으로 사용해야 하는 데이터를 저장하는 저장소로
		session을 사용한다. request객체는 하나의 요청만 처리하는데 사용한다면 session객체는 웹브라우저의 여러 요청을 처리하는 jsp페이지에서 공유된다.
		로그인한 회원정보 등 웹브라우저와 일대일로 관련된 값을 저장할 때에는 cookie보다 session을 사용하는 것이 좋다.
	
	C. cookie vs session
		
		cookie 대신 session을 사용하는 가장 큰 이유는 session이 cookie보다 보안에 더 안정적이다는 점이다.
		cookie의 이름이나 값은 network을 통해서 전달되기 때문에 http프로토콜을 사용하는 경우 중간에 cookie값을 해킹할 수 있지만
		session은 오직 웹서버에만 저장되기 때문에 중요한 자료를 보관할 때 알맞은 저장소가 된다.
		
		session을 사용하는 두번째 이유는 웹브라우저가 cookie를 지원하지 않을 경우 or 강제적으로 cookie를 사용하지 못하게 할 경우 session은 cookie 설정여부와
		상관없이 사용할 수가 있다.
			
		1. session 종료
		
			session을 유지할 필요가 없다면 session.invalidate()메서드를 사용해서 session을 종료한다.
			session을 종료하려면 사용중인 session객체를 삭제하고 session객체에 저장되었던 속성정보도 동시에 삭제가 된다.
			
		2. session 유효시간
		
			session은 최종 접속시간을 가지고 있는데 session객체를 사용할 때마다 최종 접속시간이 갱신된다.
			session은 마지막 접근 이후 일정시간 내에 다시 session에 접근하지 않을 경우에는 자동적으로 session을 종료하는 기능을 가지고 있다.
		...
		3. request.getSession()메서드를 이용한 세션생성
			...
			이 메서드는 현재의 요청과 관련된 session객체를 리턴하는데 session이 존재하면 해당 session을 리턴하고 없다면 새로운 session을 생성해서 리턴한다.
