...
<!-- 
	A. 쿠키(cookie)사용하기
		...
		쿠키는 웹브라우저가 저장하는 데이터이다. 웹브라우저는 웹서버에 요청을 보낼 때 쿠키를 함께 전송하고 
		웹서버는 웹브라우저가 전송한 쿠키를 사용해서 데이터를 처리할 수 있다.
		쿠키는 웹서버와 웹브라우저 양쪽에 생성할 수 있는데 JSP에서 생성하는 쿠키는 웹서버에 저장하는 쿠키이다.
	
	B. 쿠키의 동작 3단계
	
		1. 쿠키생성단계 : 쿠키를 사용하려면 먼저 쿠키를 생성해야 하고 jsp프로그램에서 쿠키는 웹서버에서 생성한다.
						  생성한 쿠키는 응답데이터의 헤더에 저장해서 웹브라우저로 전송한다.
		2. 쿠키저장단계 : 웹브라우저는 응답데이터에 포함된 쿠키를 쿠키저장소에 보관하며 쿠키의 종료여부에 따라 메모리나 파일에 저장한다.
		3. 쿠키전송단계 : 웹브라우저는 저장한 쿠키를 요청이 있을 때마다 웹서버에 전송한다.
						  웹서버는 웹브라우저가 전송한 쿠키를 사용해서 필요한 작업을 수행할 수 있다. 
	
	C. 쿠키의 구성요소
	
		1. 이름		: 각 쿠기정보를 구별하기 위해 사용되는 쿠키의 이름(key)
	...
	G. Cookie값 읽기
		...
		쿠키가 생성된 후에 쿠키를 사용할 수 있는데 웹브라우저는 요청헤더에 쿠키를 전송하며 JSP는 request.getCookie()메서드를 이용해서 쿠키값을 읽어 온다.
	...
	H. Cookie값 변경 및 삭제
		...
		쿠키가 존재하지 않으면 쿠키가 생성되기 때문에 쿠키의 존재여부를 확인한 후 존재할 경우에는 쿠키값을 변경해 주면 된다.
		쿠키의 삭제는 유효기간으로 지정하면 된다.
	
	I. Cookie의 도메인
	
		기본적으로 쿠키는 쿠키를 생성한 서버에만 전송된다. 예를들어 www.google.com에 연결해서 생성된 쿠키는 다른 사이트로 접속할 때는 전송되지 않고
		해당 사이트에 연결할 때만 전송된다.
		
		하지만, 동일 도메인을 사용하는 모든 서버에 쿠키를 전송해야할 경우가 있는 경우
		예를들어 www.google.com에서 생성된 쿠키를 mail.google.com 또는 동일 도메인에 포함된 서버에 전송할 경우가 있다.
	...
	J. 쿠키의 경로
		...
		예를들어 http://www.naver.com/basic/path2/create.jsp의 url경로에서는 basic/path2 부분이 해당 경로이다.
	...
	K. 쿠키의 유효기간
		...
		쿠키의 유효시간을 정해 놓으면 유효시간동안 쿠키가 존재하고 웹브라우저가 종료되도 유효시간이 지나지 않았다면 그 쿠키는 삭제되지 않는다.
		
		유효시간을 지정하려면 setMaxAge()메서드를 사용한다. 유효시간단위는 초 단위로 지정한다.
