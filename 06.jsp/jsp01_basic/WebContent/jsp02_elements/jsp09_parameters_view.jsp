...
%>
<!-- 
	톰캣에서는 get방식으로 요청시 전달되는 파라미터의 값을 읽어올 때 사용하는 문자셋은 기본적으로 utf-8로 전달된다. 
	그렇기 때문에 tomcat에서는 별도로 문자셋을 설정하지 않아도 utf-8문자셋을 사용한다.
	하지만 utf-8이 아닌 문자셋을 get방식으로 전달하는 경우에는 server.xml에 <Connector>태그의 useBodyEncodingForURI속성을 true로 추가하면 된다.
	
	true로 설정할 경우에는 get방식으로 전달되는 파라미터값을 읽을 때 request객체의 setCharacterEncoding("utf-8")메서드를 호출해서
	지정된 문자셋을 사용할 수 있게 한다.
