...
<!-- 
	...
	JSP는 스크립트릿과 표현식 등 스크립트코드와 html tag들의 코드가 뒤섞여서 사용하기 때문에 JSP코드는 가독성이 떨어지고 코딩하기가 매우 복잡하게 
	되어 있어서 개발하기가 편리하지 않게 되어 있다. JSP는 새로운 tag를 포함한 표준라이브러리 형태로 제공을 해서 보다 쉽게 가독성이 좋게 코딩할 수 있도록
	제공하는 기능이 있는데 JSP페이지에서 많이 사용되는 논리적인 판단(if), 반복처리(for), 포맷처리 등을 쉽게할 수 있도록 표준으로 만들어서 라이브러리형태로
	제공하는 것이 JSTL이다.
	
	jstl-??-??.jar파일을 다운로드(maven repository-jstl-1.2) 받아서 WEB-INF/lib폴더에 저장하면 jstl을 바로 사용할 수 있다.
	...
	C. 국제화 태그
		...
		국제화태그는 특정지역에 따라 알맞는 메시지를 출력해야 할 경우 사용한다. 예를 들어서 한글 브라우저로 접속하면 한글메시지를 출력하고
		영문브라우저로 접속하면 영문메시지로 출력해야할 경우 국제화태그를 사용하면 쉽게 적용할 수 있다.
		...
		Message 	bundle 			사용할 언어번들을 지정
					...
					setBundle 		리소스번들을 로딩 후에 특정 변수에 저장
		...
		1. Locale지정태그
			...
			웹브라우저는 Accept-Language헤더를 사용해서 수용가능한 언어목록을 전송하는데 JSTL국제화태그들은 이 헤더의 값을 이용해서 언어별로 알맞은 언어처리를 하게 된다. 
			예를 들어 메시지를 출력해주는 message태그는 Accept-Language의 값이 'ko'인 경우 한글 메시지를 우선적으로 처리하고 'en'인 경우 영문메시지를 우선적으로 처리한다.
			
			국제화태그가 Accept-Language헤더에서 지정한 언어가 아닌 경우 다른 언어를 사용하도록 지정할 때 사용하는 태그가 fmt:setLocale태그이다.
		
		2. Resource Bundle
		
			메시지태그에서 사용할 리소스번들파일을 작성할 때 베시지 번들파일은 classpath에 위치해야 하기 때문에 WEB-INF/classes폴더나 WEB-INF/lib에 포함된 jar파일에
			...
			예를 들어 "번들이름_국가코드.properties"의 형식으로 작성해야 하고 국가코드는 생략할 수 있다.
		...
		4. format 태그
			...
			시간대 설정 	: fmt:setTimezone, fmt:timeZone
