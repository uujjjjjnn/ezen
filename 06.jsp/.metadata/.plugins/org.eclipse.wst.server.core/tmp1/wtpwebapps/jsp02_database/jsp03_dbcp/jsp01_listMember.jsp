...
<!-- 
	...
	커넥션풀기법이란? 데이터베이스와 커넥션을 사전에 만들어서 pool에 저장해두고 필요할 때마다 풀에서 꺼내서 사용한 후에
	다시 pool에 반환하는 기법을 말한다.
	
	커넥션풀의 특징은
	
	1. 사전에 풀에 Connection이 생성되어 있기 때문에 커넥션을 생성하는데 필요한 시간을 감소시킨다.
	2. Connection을 재사용하기 때문에 생성되는 Connection갯수를 일정하게 관리할 수 있다.
	3. Connection Pool을 사용하게 되면 웹애플리케이션의 성능과 처리성능이 향상되기 때문에 웹애플리케이션 대부분이
	   커넥션풀을 기본적으로 사용하고 있다.
	
	커넥션풀을 사용하는 방법
	
	1. DBCP관련 jar파일과 JDBC관현 jar파일을 설치
