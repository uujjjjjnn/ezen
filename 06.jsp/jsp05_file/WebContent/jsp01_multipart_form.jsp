...
<!-- 
	...
	게시판이나 자료실 등에서 제품이미지 등의 파일을 업로드 할 경우 request기본객체 등에서 제공하는 기능으로 파일을 업로드 할 수가 있다.
	
	일반 파라미터로 전송할 때 사용하는 인코딩자료와 파일을 업로드할 때 사용하는 인코딩자료의 형식이 다르다.
	파일을 업로드할 경우는 스트림기반의 전송방식에는 인코딩방식에 따라 전송하는 데이터형식이 달라진다.
	
	파일을 업로드하기 위해서는 "multipart/form-data" 인코딩방식을 사용해야 하는데 데이터를 전송할때 "multipart/form-data" 방식으로 전송해야 한다.
	
	이 방법으로 전송하기 위해서는 form태그의 enctype의 속성값을 "multipart/form-data"으로 지정해야 한다.
	그리고 "multipart/form-data"인코딩방식을 post방식의 한 종류이기 때문에 method속성도 post로 설정해야 한다.
