...
/*
 	...
 	제네릭메서드는 매개타입과 리턴타입으로 타입파라미터를 갖는 메서드를 말한다. 제네릭메서드를 선언하는 방법은 리턴타입앞에 <>를 추가하고 타입파라미터를 기술한 후에 
 	리턴타입과 매개타입으로 타입파라미터를 사용하면 된다.
 	
 	public <T> 리턴타입<T> 메서드명(T, t) { return 리턴타입; }
 	
 	제네릭메서드는 2가지 방식으로 호출할 수 있다. 코딩할 때 타입파라미터의 구체적인 타입을 명시적으로 지정해도 되고,
 	컴파일러가 매개값의 타입으로 보고 구체적인 타입을 추적하도록 할 수 있다.
