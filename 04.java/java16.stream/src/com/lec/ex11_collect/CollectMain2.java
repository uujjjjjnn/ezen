...
/*
 	사용자정의 컨테이너에 수집하기
 	 ...
 	 List, Set, Map컬렉션을 사용하는 것이 아니라 사용자가 만든 컨테이너(즉, 사용자가 정의한 컬렉션)로 stream을 저장하는 방법을 실습
 	
 	 stream은 요소들을 필터링 or 매핑을 해서 사용자가 정의한 컨테이너에 요소들을 저장할 수 있도록 collection()메서드를 이용해서 
 	 사용자정의 컨테이너(MaleStudent)에 저장할 수가 있다.
...
public class CollectMain2 {
	...
	public static void main(String[] args) {
		...
		MaleStudent 남학생 = list.stream()
								...
								// 남학생2명으로 새로 MaleStudent로 만듦, m=남학생(2명)을 accumulate, m1이 추가된 리스트에 m2 추가 
