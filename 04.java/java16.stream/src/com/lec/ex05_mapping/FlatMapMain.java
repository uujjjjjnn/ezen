...
/*
 	...
 	매핑은 중간처리기능으로 스트림의 요소를 다른 요소로 대체하는 작업을 말한다. 스트림에서 제공하는 매핑메서드는 flatMapXXX(), mapXXX(), asXXXStream(), boxed()가 있다.
 	
 	1. flatMapXXX()		: 이 메서드는 요소를 대체하는 복수개의 요소들로 구성된 새로운 스트림을 리턴
 	2. mapXXX()			: 이 메서드는 요소를 대체하는 하나의 요소로 구성된 새로운 스트림으로 리턴
 	3. asXXXStream()	
 		
 		a. asDoubleStream()메서드는 IntStream의 int, LongStream의 long의 값들을 double타입으로 리턴
 		b. asLongStream()메서드는 IntStream의 int값을 long으로 타입변환 후 LongStream으로 리턴
 		
 	4. boxed() 			: int, long, double 기본타입을 각각 Integer, Long, Double으로 박싱을해서 stream으로 리턴
