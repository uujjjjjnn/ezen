...
/*
 	...
 	1. Class객체얻기(Class.forName(String str), Object.getClass(클래스이름))
 		...
 		Object는 모든 클래스의 최상위 클래스이므로 모든 클래스메서드에서 getClass()메서드를 호출할 수 있다.
 		getClass()메서드는 해당 클래스의 객체가 생성되어 있을 때에만 사용할 수 있지만, 객체가 생성되어 있지 않을 경우 Class객체를 얻을 수가 없다.
 		
 		Class의 생성자는 Private으로 선언되어 있기 때문에 new객체생성연산자로 객체를 만들 수가 없다.
 		정적인 메서드 forName()을 이용해서 클래스 정보를 얻을 수 있다. forName()메서드는 클래스전체이름 즉, 패키지이름을 포함한 전체 클래스이름을 매개값으로 전달받아서
 		해당 클래스의 객체를 생성한 후에 그 객체를 리턴한다.
 		
 		Class.forName()메서드는 클래스를 찾지 못했을 경우에 ClassNotFoundException예외를 발생시키기 때문에 예외처리를 해야 한다.
 	
 	2. Class정보얻기(Reflection)
	
		getDeclaredConstructors(), getDeclaredFields(), getDeclaredMethods() 등의 메서드를 이용해서 해당 클래스의 필드, 생성자, 메서드에 대한 정보를 얻을 수가 있다.
