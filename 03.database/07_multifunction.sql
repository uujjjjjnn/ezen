...
select min(sal + nvl(comm, 0)) "최고 급여"
  ...
  from emp;
*/
select ename 사원명, sal 급여, nvl(comm, 0) 보너스, sal + nvl(comm, 0) 합계 from emp
...
select ename 사원명, sal 급여, nvl(comm,0) 보너스, round(avg(sal), 1) 평균 from emp
...
select deptno 부서
      ...
      , sal 급여
			, nvl(comm, 0) 보너스
			, sum(sal + nvl(comm, 0)) 
			  over(partition by deptno order by sal + nvl(comm, 0)) total
  from emp;
...
select deptno "부서"
		 , ename "이름"
		 , sal "급여"
		 , sum(sum(sal)) over(partition by deptno) "부서급여합계"
