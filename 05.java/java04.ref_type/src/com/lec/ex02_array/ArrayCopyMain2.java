...
/*
 	...
 	참조타입인 배열인 경우, 배열복사가 되면 복사되는 값이 객체의 메모리주소이기 때문에 새 배열의 항목은 이전 배열이 참조하는 객체의 메모리 주소와 동일하다.
 	
 	이 것을 얕은 복사(shallow copy)라고 한다. 반대로 깊은 복사(deep copy)는 참조하는 객체를 별도로 생성해서 복사하는 것을 말한다.
 	
 	Array복사
 	
 	자바에서는 배열을 한번 생성하면 크기를 변경할 수 없기 때문에 더 많은 요소를 저장하려면 보다 큰 배열을 새로 만들고 이전 배열에서 항목값들을 복사해야 한다.
 	배열간 항목을 복사하려면 for문을 이용하거나 System.arraycopy(), Arrays.copyOf()메서드를 사용해서 깊은 복사를 하게 한다.
...
public class ArrayCopyMain2 {
	...
	public static void main(String[] args) {
		...
		System.out.print("old2 array = ");
		for(String o : old2) {
			System.out.print(o + ", ");
		}
		System.out.println();
		System.out.print("new2 array = ");
		for(String o : new2) {
			System.out.print(o + ", ");
		}
		System.out.println("\n----------------------------------------");
		...
		System.out.print("old3 array = ");
		for(int o : old3) {
			System.out.print(o + ", ");
		}
		System.out.println();
		System.out.print("new3 array = ");
		for(int o : new3) {
			System.out.print(o + ", ");
		}
		System.out.println("\n----------------------------------------");
