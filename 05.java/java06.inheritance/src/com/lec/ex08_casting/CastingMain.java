...
/*
 	...
 	형변환이 된다고해서 모든 부모타입이 자식타입으로 강제형변환할 수 잇는 것은 아니다.
 	
 	강제타입변환이 되는 경우는 자식타입이 부모타입으로 형변환이 된 후에 자식타입으로 변환되는 경우에만 
 	강제타입변환을 할 수가 있다.
 	
 	자식타입이 부모타입으로 자동형변환하면 부모타입에 선언된 필드와 메서드만 사용할 수가 있다.
 	자식타입이 부모타입으로 자동형변환이 되면 부모타입에서 선언된 필드와 메서드만 사용할 수 있다.
 	만약, 자식타입에 선언된 필드와 메서드를 꼭 사용해야하는 경우에는 강제형변환을해서 
 	다시 자식타입으로 변환한 후에 자식타입의 필드와 메서드를 사용할 수 있다.
...
public class CastingMain {
	...
	public static void main(String[] args) {
		// 1. 자동형변환
		Parent parent = new Child(); 	// 생성과 즉시 자동형변환
		parent.field1 = "사용가능";
		...
		// false일 경우에는 강제형변환되지 않도록 로직에 반영해야 한다.
