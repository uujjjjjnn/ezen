...
/*
 	...
 	스트림은 요소들을 필터링 or 매핑한 후에 요소들을 수집하는 최종저리 메서드인 collect()를 제공한다.
 	Stream의 collect(Collectors<T,A,R> collector)메서드는 필터링 or 매핑된 요소들을 새로운 컬렉션타입으로 수집하고 그 결과인 Collection(List, Set, Map)으로 리턴한다.
 	
 	매개값인 Collectors는 어떤 요소를 어떤 컬렉션타입으로 수집할 것인지를 정의한다.
 	Collector의 타입 파라미터인 T는 스트림의 요소, A는 누적기(accumulator), R은 스트림요소들이 저장될 컬렉션을 의미한다.
 	
 	리턴값인 Collector에는 Collectors<T, ? R>처럼 누적기(A)가 ?로 되어 있는 경우도 있는데 
 	이 의미는 Collector가 T(스트림의 요소)를 R(컬렉션)에 저장하는 방법을 알고 있어서 A가 필요없다는 의미이다.
...
public class CollectMain1 {
	...
	public static void main(String[] args) {
		...
		여학생 = list.stream()
				.filter(s -> s.getGender() == Student.Gender.FEMALE)
				.collect(Collectors.toCollection(() -> {return new HashSet<Student>();})); // 람다식으로 생성자호출 
		여학생.stream().forEach(s -> System.out.print(s.getName() + ", "));
		System.out.println();
	}
