...
/*
 	...
 	집계(aggregate)는 최종처리기능으로 요소들을 처리해서 갯수, 합계, 평균, 최대/최소값등과 같이 하나의 값으로 집계하는 기능을 말한다.
 	
 	집계는 대용량의 데이터를 가공해서 축소하는 reduction이라고 볼 수 있다.
 	스트림이 제공하는 기본집계 메서드는 
 	
 	1. count() 				: 요소의 갯수를 long타입으로 리턴
 	2. findFirst()			: 첫 번째 요소를 OptionalXXX타입으로 리턴
 	...
 	이 집계요소에서 리턴하는 OptionalXXX클래스는 Java8에서 추가한 java.util 패키지의 Optional, OptionalInt, OptionalLong, OptionalDouble클래스 타입을 말한다.
 	이들 객체에서 값을 얻기 위해서는 각각 get(), getAsDouble(), getAsInt(), getAsLong()메서드를 호출하면 된다.
 	
 	이 OptionalXXX클래스들은 저장하는 값의 타입만 다를 뿐 제공하는 기능은 거의 유사하다.
 	Optional클래스는 단순히 집계만 제공하는 것이 아니라 집계값이 존재하지 않을 경우 디폴트값을 설정할 수 있고 집계값을 처리하는 Consumer클래스도 등록할 수 있다.
...
public class AggregateMain {
	...
	public static void main(String[] args) {
		...
		System.out.println("2의 배수의 최대값 = " + Arrays.stream(int_arr).filter(n -> n%2==0).max().getAsInt());
		System.out.println();
		
		// 5. min
		System.out.println(Arrays.stream(int_arr).filter(n -> n%2==0).min().getClass());
		System.out.println("2의 배수의 최소값 = " + Arrays.stream(int_arr).filter(n -> n%2==0).min().getAsInt());
		System.out.println();
		
		// 6. findFirst
		System.out.println(Arrays.stream(int_arr).filter(n -> n%2==0).findFirst().getClass());
		System.out.println("2의 배수의 첫 번째 값 = " + Arrays.stream(int_arr).filter(n -> n%2==0).findFirst().getAsInt());
		System.out.println();
