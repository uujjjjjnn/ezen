...
/*
 	...
 	IO패키지에서 제공하는 File클래스는 파일크기, 속성, 이름 등의 정보를 읽는 기능과 파일생성, 삭제 등 관련 기능을 제공한다.
 	또한 디렉토리를 생성하고 디렉토리에 있는 파일목록을 읽는 기능도 제공한다.
 	
 	하지만,파일의 데이터를 읽거나 쓰는 기능을 지원하지 않는다. 파일에 데이터를 입출력하고자할 경우에는
 	File입출력스트림을 사용해야 한다.
 	
 	디렉토리구분자는 윈도우에서는 '\' or '/' linux에서는 '/'을 사용한다. 만약에 '\'을 사용할 경우에는 이스케이프문자('\\')로 사용해야 한다.
 	...
 	* 파일 or 디렉토리가 존재할 경우 관련 메서드
 	...
 	6. gerParentFile() : 부모디렉토리를 File객체로 생성한 후 리턴
 	7. getPath() : 전체경로리턴
