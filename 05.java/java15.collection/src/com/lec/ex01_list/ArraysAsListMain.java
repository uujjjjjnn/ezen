...
public class ArraysAsListMain {
	...
	public static void main(String[] args) {
		...
		// ArrayList에는 초기화기능이 없다. 대신에 Arrays.asList(값,..)메서드를 이용해서 초기화시킬 수 있다.
		// ArrayList를 생성하고 Runtime시에 필요에 따라 객체를 추가하는 방법이 일반적이지만
		// 고정된 객체로 구성된 List를 생성할 경우에는 Arrays.asList()메서드를 사용하는 것이 편리하다.
		// 다만 Arrays.asList()로 생성된 ArrayList객체는 더이상 크기를 변경할 수 없다.
		...
		// list2.remove(0);  (X) 삭제불가, UnsupportedOperationException 예외 발생
